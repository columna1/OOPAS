...
local function readReplay(filename)
	...
	if file then
		...
		replay.compressedData = read.readBytes(len)
		replay.uncompressedData = lzma.uncompress(lzma_format(replay.compressedData))
		replay.onlineID = read.readLong()
		read.file = nil
	else
		error("Couldn't open replay file")
	end
	file:close()
	return replay
end
